...
In a single pass, this script:
  ...
  2. Identifies dependent views across ALL Snowflake databases (cross-database support),
...
def find_impacted_views_cross_db(cs, deployed_database, deployed_tables):
    """Identifies dependent views across ALL databases in Snowflake."""
    impacted = {}  # (view_db, view_schema, view_name) -> set of (table, method)

    # Method 1: Account-Wide Lineage Lookup (Supports Cross-Database)
    for full_name in deployed_tables:
        ...
        try:
            ...
            for r_db, r_schema, r_view in cs.fetchall():
                impacted.setdefault((r_db, r_schema, r_view), set()).add((full_name, "lineage"))
        except Exception as exc:
            logger.warning(f"Lineage lookup skipped for {full_name}: {str(exc).splitlines()[0]}")

    # Method 2: Account-Wide Text Scan Fallback (SNOWFLAKE.ACCOUNT_USAGE.VIEWS)
    ...
    if table_short_names:
        ...
        try:
            ...
            for v_db, v_schema, v_name, definition in cs.fetchall():
                if not definition:
                    continue
                for short_name, pattern in patterns.items():
                    if pattern.search(definition):
                        ...
                        impacted.setdefault((v_db, v_schema, v_name), set()).add((matched_full, "text scan"))
        except Exception as exc:
            logger.warning(f"Cross-database text scan fallback skipped: {str(exc).splitlines()[0]}")
...
def run_validate_views(cs, args, deployed_tables):
    ...
    impacted = find_impacted_views_cross_db(cs, args.snowflake_database, deployed_tables)
    if not impacted:
...
def main():
    ...
    try:
        ...
        # 2. Validate cross-database impacted views and generate CSV
