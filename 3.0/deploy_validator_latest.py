...
def run_extract_tables(cs, snowflake_database, change_history_table_param, build_id):
    """Extracts deployed tables for a build and returns them as a sorted list."""
    logger.info("--- STARTING TABLE EXTRACTION ---")

    change_history_table = get_change_history_table_details(snowflake_database, change_history_table_param)
    ...
    script_paths = get_applied_script_paths(cs, change_history_table, build_id)
...
def run_validate_views(cs, snowflake_database, validate_databases, report_file, deployed_tables):
    ...
    if validate_databases:
        logger.info(f"Filtering view discovery to databases: {', '.join(validate_databases)}")
    else:
    ...
    impacted = find_impacted_views_cross_db(
        ...
        snowflake_database, 
        deployed_tables, 
        target_databases=validate_databases
    )
...
def deploy_validator(snowflake_account, snowflake_user, snowflake_role, snowflake_warehouse, 
                     snowflake_database, change_history_table, build_id, report_file, validate_databases=None):
    ...
    try:
        ...
        run_validate_views(cs, snowflake_database, validate_databases, report_file, deployed_tables)
    finally:
        cs.close()
...
if __name__ == "__main__":
    ...
    deploy_validator(
        ...
        validate_databases=args.validate_databases
    )
