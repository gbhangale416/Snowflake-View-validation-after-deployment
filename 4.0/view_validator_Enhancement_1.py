...
# =============================================================================
# ENHANCEMENT 4: ERROR CATEGORIZATION
# =============================================================================

def categorize_snowflake_error(error_message):
    """Classifies Snowflake SQL error messages into clear category tags."""
    msg = error_message.upper()
    
    if "INVALID IDENTIFIER" in msg or ("COLUMN" in msg and "DOES NOT EXIST" in msg):
        return "MISSING_COLUMN"
    elif "DOES NOT EXIST OR NOT AUTHORIZED" in msg:
        return "OBJECT_NOT_FOUND_OR_NO_ACCESS"
    elif "SYNTAX ERROR" in msg:
        return "SYNTAX_ERROR"
    elif "INSUFFICIENT PRIVILEGES" in msg or "ACCESS CONTROL" in msg:
...
def run_extract_tables(cs, snowflake_database, change_history_table, build_id):
    ...
    logger.info(f"Tables touched by this deployment to {snowflake_database}: {', '.join(deployed_tables) or '(none)'}")

    print(f"deployed tables: {deployed_tables}")
...
def find_impacted_views_cross_db(cs, deployed_database, deployed_tables, target_env=None):
...
def validate_views_cross_db(cs, impacted):
    """Validates views across databases with SELECT LIMIT 0 and categorizes error messages."""
    results = []
    failed = []
    total = len(impacted)
    ...
    for idx, ((v_db, v_schema, v_view), sources) in enumerate(sorted(impacted.items()), start=1):
        ...
        try:
            ...
            results.append((v_db, v_schema, v_view, source_desc, "OK", "", "NONE"))
            logger.info(f"[{idx}/{total}] OK - {v_db}.{v_schema}.{v_view}")
        except Exception as exc:
            raw_err_msg = str(exc).replace("\n", " ").replace("\r", " ")
            category = categorize_snowflake_error(raw_err_msg)
            formatted_err = f"[{category}] {raw_err_msg}"
            
            results.append((v_db, v_schema, v_view, source_desc, "FAILED", formatted_err, category))
            failed.append((v_db, v_schema, v_view, formatted_err))
            logger.error(f"[{idx}/{total}] FAILED - {v_db}.{v_schema}.{v_view}: {formatted_err}")

    return results, failed


def create_csv_report(results, report_file):
    """Creates and saves the validation CSV report including category tags."""
    os.makedirs(os.path.dirname(os.path.abspath(report_file)), exist_ok=True)
    with open(report_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["View_database", "Schema", "View", "Deployed_tables_referenced", "Status", "Error", "Error_Category"])
        writer.writerows(results)
    logger.info(f"CSV validation report successfully written to: {report_file}")


# =============================================================================
# ENHANCEMENT 3: AZURE DEVOPS SUMMARY REPORT
# =============================================================================

def create_azure_devops_summary(results, report_file):
    """Generates a Markdown summary and attaches it to the Azure DevOps Pipeline run page."""
    summary_file = report_file.rsplit('.', 1)[0] + "_summary.md"
    os.makedirs(os.path.dirname(os.path.abspath(summary_file)), exist_ok=True)
    
    total = len(results)
    failed_count = sum(1 for r in results if r[4] == "FAILED")
    passed_count = total - failed_count

    md_lines = [
        "## 🔍 View Validation Summary Report",
        f"**Total Views Checked:** {total} | **Passed:** ✅ {passed_count} | **Failed:** ❌ {failed_count}",
        "",
        "| View Database | Schema | View Name | Status | Category | Error Details |",
        "| --- | --- | --- | --- | --- | --- |"
    ]
    ...
    for v_db, schema, view, sources, status, err, category in results:
        ...
        md_lines.append(f"| `{v_db}` | `{schema}` | `{view}` | {status_icon} | `{category}` | {clean_err} |")

    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))

    # Publish Markdown summary directly into the Azure DevOps Pipeline Run UI
    print(f"##vso[task.uploadsummary]{os.path.abspath(summary_file)}")
    logger.info(f"Azure DevOps Markdown summary published: {summary_file}")


def run_validate_views(cs, snowflake_database, report_file, deployed_tables, target_env=None):
    """Validates cross-database views impacting the deployed tables and generates CSV + Markdown reports."""
    ...
    logger.info(f"Tables deployed to {snowflake_database}: {', '.join(deployed_tables)}")
    ...
    # Generate Azure DevOps Summary Report (Enhancement 3)
    create_azure_devops_summary(results, report_file)
...
def view_validator(snowflake_account, snowflake_user, snowflake_role, snowflake_warehouse,
    ...
    try:
        ...
        deployed_tables = run_extract_tables(cs, snowflake_database, change_history_table, build_id)

        # 2. Validate impacted views for specified databases and generate CSV
        run_validate_views(cs, snowflake_database, report_file, deployed_tables, target_env=target_env)
    finally:
        cs.close()
