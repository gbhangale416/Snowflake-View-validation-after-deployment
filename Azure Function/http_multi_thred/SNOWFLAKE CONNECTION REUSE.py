...
def get_thread_snowflake_cursor():
    ...
    try:
        logging.info("🔌 [SNOWFLAKE] Initializing new thread-local connection...")
...
def fetch_views_for_db(target_db, target_schemas):
    """Batch discovers views inside target_db using persistent connection."""
...
def validate_single_view(view_tuple):
    """Executes 'SELECT * LIMIT 0' health check reusing persistent connection with single retry."""
...
# =============================================================================
# MAIN TRIGGER FUNCTION (WITH TOP-LEVEL TRY-CATCH)
# =============================================================================
    ...
    try:
        ...
        if not metameta_name:
            try:
                ...
                logging.info("ℹ️ [PARAMS] No JSON body supplied. Using default/fallback metadata name.")

        if not metameta_name:
            metameta_name = "View_validation"

        logging.info(f"📋 [PARAMS] Target Metameta Name: '{metameta_name}'")

        try:
            parallel_workers = int(os.environ.get("PARALLEL_WORKERS", "10"))
        except ValueError:
            logging.warning("⚠️ [CONFIG] Invalid PARALLEL_WORKERS value in env. Defaulting to 10.")
            parallel_workers = 10
        ...
        # 3. Discover Views Parallelly
        all_views = []
        try:
            logging.info(f"⚡ [THREADING] Starting view discovery thread pool (max_workers={parallel_workers})...")
            ...
            logging.error(f"❌ [DISCOVERY] Error during parallel view discovery pool: {exc}", exc_info=True)
        ...
        # 4. Parallel View Health Check Validation
        all_results = []
        failed_results = []

        try:
            logging.info(f"⚡ [THREADING] Starting parallel view health checks for {len(all_views)} view(s)...")
        ...
        # Catch-all top-level exception handler
        ...
        # Guarantee cleanup of thread-local resources
