...
def send_teams_notification(webhook_url, metameta_name, total_views, passed_cnt, failed_cnt, failed_list=None):
    ...
    if is_success:
        ...
        if failed_cnt > 5:
            failure_details += f"\n*...and {failed_cnt - 5} more. See attached CSV report for full details.*"
...
def send_email_notification(metameta_name, total_views, passed_cnt, failed_cnt, csv_data=None, filename=None):
    """Sends email notification. Attaches CSV only if failed views exist."""
    ...
    if is_success:
        ...
        ATTENTION: Attached is the CSV report listing all FAILED views and error details.
...
def generate_failed_csv_report(failed_results):
    ...
    # Group failed results by Database
    grouped_failures = {}
    ...
    # Write each database section into CSV
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if failed_count > 0:
        ...
        # Send failure notifications via Email and Teams with CSV
        ...
        # --- SUCCESS PATH ---
        # Send success notifications via Email and Teams (no attachment needed)
        send_teams_notification(
            webhook_url=teams_webhook_url,
            metameta_name=metameta_name,
            total_views=total_count,
