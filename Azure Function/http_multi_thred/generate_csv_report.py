...
def generate_csv_report(results):
    """Generates standard CSV content with Database column."""
    output = io.StringIO()
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not all_views:
        return func.HttpResponse(
            ...
            status_code=200,
            mimetype="text/csv",
            headers={"Content-Disposition": f"attachment; filename=view_validation_report_{metameta_name}.csv"}
        )
    ...
    # -------------------------------------------------------------------------
    # 4. Generate & Return CSV
    # -------------------------------------------------------------------------
    csv_data = generate_csv_report(results)
    filename = f"view_validation_report_{metameta_name}.csv"
