...
import logging
import os
import time  # 1. Imported time module
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logging.info(f"Loaded '{clean_name}' from Azure Blob Storage.")
        ...
        if found_path:
            ...
            logging.info(f"Loaded '{clean_name}' from local file '{found_path}'.")
        else:
            return func.HttpResponse(
                json.dumps({
                    "status": "ERROR", 
                    "message": f"Metameta file '{clean_name}' not found in Azure Blob Storage or locally."
                }),
                status_code=404,
                mimetype="application/json"
            )
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps({"status": "ERROR", "message": f"Failed to parse metameta JSON: {str(exc)}"}),
            status_code=500,
            mimetype="application/json"
        )
    ...
    if not all_views:
        # Calculate execution time for empty scenario
    ...
    # -------------------------------------------------------------------------
    # 5. Calculate Duration & Construct Response Payload
    # -------------------------------------------------------------------------
