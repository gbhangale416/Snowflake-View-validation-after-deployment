...
def extract_all_db_schema_targets(metadata):
    ...
    for ent in entities:
        ...
        # Per-Entity View Exclusions (Handles missing keys, None, or empty lists)
...
def fetch_views_for_db(cs, target_db, target_schemas, excluded_views=None):
    """
    Discovers views using target_db.INFORMATION_SCHEMA.VIEWS
    filtered by target schemas and per-entity excluded views. Reuses active cursor (cs).
    """
...
def validate_single_view(cs, view_tuple):
    """Executes 'SELECT * LIMIT 0' health check on a single view using active cursor (cs)."""
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Open SINGLE Snowflake Connection for sequential execution
        ...
        try:
            ...
            for db, schemas in all_db_schema_map.items():
                ...
                db_views = fetch_views_for_db(cs, db, schemas, db_excluded_views)
            ...
            logging.info(f"Starting sequential validation for {len(all_views)} views...")
            all_results = []
            failed_results = []

            for view_tuple in all_views:
                res = validate_single_view(cs, view_tuple)
                all_results.append(res)
                if res[3] == "FAILED":
                    failed_results.append(res)

        finally:
            # Safely close the single connection
            cs.close()
            ctx.close()
