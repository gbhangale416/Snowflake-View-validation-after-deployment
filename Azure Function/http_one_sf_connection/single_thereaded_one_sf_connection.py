...
def fetch_views_for_db(cs, target_db, target_schemas):
    """
    ...
    Reuses the existing Snowflake cursor (cs).
    """
...
def validate_single_view(cs, view_tuple):
    """Executes 'SELECT * LIMIT 0' health check on a single view using shared cursor (cs)."""
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Open SINGLE Snowflake Connection for the entire run
        ...
        try:
            ...
            for db, schemas in all_db_schema_map.items():
                db_views = fetch_views_for_db(cs, db, schemas)
            ...
            # Clean up the single connection safely
            cs.close()
            ctx.close()
