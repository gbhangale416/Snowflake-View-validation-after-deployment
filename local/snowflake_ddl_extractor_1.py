...
def extract_target_tables_tokens(content: str, dialect: str = "snowflake") -> list[dict[str, str]]:
    ...
    Tokenizes Snowflake SQL directly to extract target tables across non-standard DDL syntax.
    """
    tables = []
    try:
    ...
    while i < num_tokens:
        ...
        # Look for DDL start keywords
        if token.token_type in ddl_triggers:
            j = i + 1

            # Advance past optional keywords (OR, REPLACE, TRANSIENT, TEMPORARY, etc.)
            ...
            # Verify if target keyword is TABLE
            if j < num_tokens and (tokens[j].token_type == TokenType.TABLE or tokens[j].text.upper() == "TABLE"):
                ...
                # Collect full table identifier tokens (e.g. DB . SCHEMA . TABLE)
                name_parts = []
                while k < num_tokens and tokens[k].token_type in {
                    TokenType.VAR,
...
def extract_deployed_tables(path_or_paths, dialect: str = "snowflake") -> list[dict[str, str]]:
    """Extracts target table details using SQLGlot AST with Tokenizer fallback."""
    ...
    for path in script_paths:
        try:
            ...
            # 2. If AST returned no results (due to syntax issues or non-standard DDL), fallback to Tokenizer
